...
function Bot(name, password, host, port) {// password not needed
    ...
    process.env.NODE_ENV === 'production' && axios.post('https://evve.herokuapp.com/api/notify', { title: 'rbot', description: 'rbot joined the world' })
...
function onend(reason) {
    process.env.NODE_ENV === 'production' && axios.post('https://evve.herokuapp.com/api/notify', { title: 'rbot left', description: reason || 'rbot left the world' })
}
